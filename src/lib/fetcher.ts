...
export const fetcher = async (
	...
	const results = await fetch('https://apis.raster.app', {
		method: 'POST',
		headers: {
...
export const swrLibreriesFetcher = (key: Fetcher) =>
	fetcher(key.query, key.variables, key.settings).then((res) => {
		const libraryList = res.libraries.map((library: Library) => ({
			id: library.id,
			name: library.name,
			photosCount: library.photosCount,
		}))
 ...
 */
export const swrImagesFetcher = (key: Fetcher) =>
	fetcher(key.query, key.variables, key.settings).then((res) => res.photos)
